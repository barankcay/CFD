...
for i in range(1, N + 1):
    if i == 1:
        ...
        eqn = (k * a * ((Temp[N + 1] - Temp[N]) / (d / 2)) - k * a * ((Temp[N] - Temp[N - 1]) / d)) + S * V+rho*cp*U*a*Temp[i-1] -rho*cp*U*a*Temp[i]
        coefficients = collect(eqn, Temp[1:N+1], evaluate=False)
        constantMatrix.append(coefficients[1])
        # print(constantMatrix)
        ...
        eqn = (k * a * ((Temp[i + 1] - Temp[i]) / d) - k * a * ((Temp[i] - Temp[i - 1]) / d)) + S * V +rho*cp*U*a*Temp[i-1]-rho*cp*U*a*Temp[i]
        coefficients = collect(eqn, Temp[1:N+1], evaluate=False)
        constantMatrix.append(coefficients[1])
        # print(constantMatrix)
