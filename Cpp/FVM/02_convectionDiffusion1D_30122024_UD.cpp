...
int main()
{
    ...
    const double velocity = 0.3; // constant velocity field value. for only diffusion solving, it can be set to 0
    ...
    for (int i = 0; i < nCell; i++) // assignin coefficients of cells and creating coefficient matrix, B
    {
        if (i == 0)
        {
            ...
            aL[0][i] = 0;
            aR[0][i] = D[0][i + 1] * A[0][i + 1] + max(-F[0][i + 1], 0.0);
            SP[0][i] = -(2 * D[0][i] * A[0][i] + max(F[0][i], 0.0));
            SU[0][i] = Ta * (2 * D[0][i] * A[0][i] + max(F[0][i], 0.0)) + source * V[0][i];
        ...
        {
            // assignin coefficients of cells
            aL[0][i] = D[0][i] * A[0][i] + max(F[0][i], 0.0);
            aR[0][i] = 0;
            SP[0][i] = -(2 * D[0][i + 1] * A[0][i + 1] + max(-F[0][i + 1], 0.0));
            SU[0][i] = Tb * (2 * D[0][i + 1] * A[0][i + 1] + max(-F[0][i + 1], 0.0)) + source * V[0][i];
            aP[0][i] = aL[0][i] + aR[0][i] + (F[0][i + 1] - F[0][i]) - SP[0][i];
